...
test("offers a DB payload reference when the service name matches a DB engine with one", () => {
  ...
  expect(screen.getByText("DB 페이로드 참고 열기")).toBeTruthy();
  fireEvent.click(screen.getByText("DB 페이로드 참고 열기"));
...
test("does not offer a DB payload reference for a service with none, or with no serviceName given", () => {
  const {rerender} = render(<ServiceCommandSession commands={[command]} serviceKey="ftp:21"
    targetIp="10.10.10.10" port={21} protocol="tcp" serviceName="ftp" onReview={vi.fn()} />);
  expect(screen.queryByText("DB 페이로드 참고 열기")).toBeNull();
