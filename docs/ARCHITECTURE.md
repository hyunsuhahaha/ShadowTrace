...
백엔드 기능은 도메인별 모듈로 분리한다. 모듈 간 참조는 정수 ID와 서비스 계층을 통해
이루어지며 다른 모듈의 저장소 구현에 직접 의존하지 않는다. 자동화 제한(취약점 자동
판정 금지, 공격 명령 자동 선택 금지, 자동 셸 획득 금지, AI 분석/결론 생성 금지 등)은
모든 모듈에 동일하게 적용되는 제품 경계이며 `backend/app/product_policy.py`와
`docs/OSCP_POLICY.md`에 코드/정책 양쪽으로 고정돼 있다.

현재 실제 모듈 목록·파일 구성·라우트 prefix는 이 문서에서 따로 유지하지 않는다 —
파일 단위로 갱신되는 살아있는 문서는
[`docs/ENGINEERING_ONBOARDING.md`](ENGINEERING_ONBOARDING.md) §6(기능별 코드 위치)과
§11(백엔드 모듈 파일 구성)이며, 새 모듈/라우트가 추가될 때마다 그쪽만 갱신하면 된다.
표를 이 문서에도 중복 유지하면 다시 벌어지므로(과거 사례는 §11.1 참고) 여기서는 설계
원칙만 남긴다.

신규 기능은 `backend/app/modules/<module>`과
